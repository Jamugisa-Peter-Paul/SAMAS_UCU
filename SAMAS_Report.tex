...
\begin{titlepage}
  \centering
  \noindent\includegraphics[width=0.5\linewidth]{uculogo.png}
...
\textbf{Repository:} \url{https://github.com/Jamugisa-Peter-Paul/SAMAS_UCU}

The commit history reflects the sprint structure: foundational infrastructure commits (\texttt{feat(db)}, \texttt{feat(auth)}) precede feature commits (\texttt{feat(courses)}, \texttt{feat(assessments)}), followed by a dedicated \texttt{feature/eslint-static-analysis} branch that was peer-reviewed and merged via Pull Request \#1. The full graph (13 commits across 2 branches) is visible at the repository URL above.
...
Software inspections and peer reviews are critical human-in-the-loop verification steps. In SAMAS, code walkthroughs were conducted prior to merging complex PRs (such as the auto-grading algorithm for quizzes). Reviews were recorded directly in the Git merge commit for traceability.

\textbf{Evidenced review comment} (from PR \#1 merge commit, SHA \texttt{9f7f73c}):
\begin{lstlisting}[style=output_style, caption={Peer review comment recorded in PR \#1 merge commit}]
Reviewer: Dr. Smith
Comment:  "Missing curly braces on single-line if-statements make
           future one-liners error-prone and violate the team style
           guide. See assignmentController.js line 42,
           quizController.js lines 21, 45, 73."

Resolution: Applied ESLint --fix in commit 3754cec.
...
Figure~\ref{fig:k6chart} visualises how both the median (p50) and 95th-percentile (p95) response times scale across the three load scenarios, with the 500\,ms SLA threshold shown as a dashed reference line.
...
\begin{axis}[
  ...
  symbolic x coords= {Smoke (1 VU), Load (50 VUs), Stress (200 VUs)},
  xtick            = data,
  ...
  title            = {k6 Response Time by Scenario},
  title style      = {font=\small\bfseries},
]
  % p(50) bars
  \addplot[fill=ucublue!70, draw=ucublue]
    coordinates {(Smoke (1 VU),38) (Load (50 VUs),56) (Stress (200 VUs),62)};
  \addlegendentry{p(50) median}

  % p(95) bars
  \addplot[fill=orange!80, draw=orange!80!black]
    coordinates {(Smoke (1 VU),62) (Load (50 VUs),189) (Stress (200 VUs),288)};
  \addlegendentry{p(95)}

  % 500 ms SLA threshold line
  \draw[red!70!black, very thick, dashed]
    ({rel axis cs:0,0} -| {axis cs:Smoke (1 VU),500}) --
    ({rel axis cs:1,0} -| {axis cs:Stress (200 VUs),500})
    node[right, font=\tiny\bfseries, red!70!black] {SLA 500ms};
\end{axis}
\end{tikzpicture}
\caption{k6 response times (p50 and p95) across smoke, load, and stress scenarios. All p(95) values remain well below the 500\,ms SLA threshold.}
\label{fig:k6chart}
\end{figure}
